...
import { accountVendingPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [accountVendingPlugin],
    });

    await request(server).get('/api/account-vending/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/account-vending/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/account-vending/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/account-vending/todos/${createdTodoItem.id}`)
